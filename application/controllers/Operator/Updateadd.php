...
class Updateadd extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->session->userdata('role_id') == 3) {
            ...
            $data['value_input'] = $this->System_model->get_value_index_add($stationid, $userid);
            $this->load->view('templates/main_header', $data);
            $this->load->view('templates/operator_sidebar', $data);
            $this->load->view('templates/main_topbar', $data);
            $this->load->view('operator/updateadd', $data);
    ...
    public function updateIndexAdditional()
    {
        $userid = $this->session->userdata('id');
        $stationid = $this->session->userdata('stationid');
        $cek = $this->db->get_where('index_add_station', ['user_id' => $userid, 'station_id' => $stationid])->num_rows();
        ...
        $this->db->update('index_add_station', $data);
        $this->session->set_flashdata('success', 'Values has been updated');
        redirect('Operator/Updateresult');
    }
}
